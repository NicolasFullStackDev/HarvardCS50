...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++)
    {
        if (strcmp(candidates[i].name, name) == 0) // Check if name matches
        {
            preferences[voter][rank] = i; // Record candidate index
            return true;
        }
    }
...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            ...
            if (!candidates[candidate_index].eliminated) // Candidate is not eliminated
            {
